...
```{r}
#| label: clean workspace & save raw data

# removes all objects except the data objects from the workspace; run if you ran the data aquisition chunk above to pull the latest raw data from EDI
# rm(list = tail(ls(), n = 14)) 

# save raw data
saveRDS(dt1, "~/Documents/projects/ds4eeb/Pete_local_data/dt1.rds")
saveRDS(dt2, "~/Documents/projects/ds4eeb/Pete_local_data/dt2.rds")
saveRDS(dt3, "~/Documents/projects/ds4eeb/Pete_local_data/dt3.rds")
saveRDS(dt4, "~/Documents/projects/ds4eeb/Pete_local_data/dt4.rds")
saveRDS(dt5, "~/Documents/projects/ds4eeb/Pete_local_data/dt5.rds")
```
...
```{r}
#| label: retrieve raw data
# retrieve raw DJFMP data from local repository

dt1 <- readRDS("~/Documents/projects/ds4eeb/Pete_local_data/dt1.rds") # trawl 1978-2001
dt2 <- readRDS("~/Documents/projects/ds4eeb/Pete_local_data/dt2.rds") # trawl 2002-2025
dt3 <- readRDS("~/Documents/projects/ds4eeb/Pete_local_data/dt3.rds") # beach seines
dt4 <- readRDS("~/Documents/projects/ds4eeb/Pete_local_data/dt4.rds") # names and tagging meta data
dt5 <- readRDS("~/Documents/projects/ds4eeb/Pete_local_data/dt5.rds") # GIS data for locations & stations
...
## location data ----
head(dt5) # lat lon data for Location & StationCode

```

Pull out the beach seine data.
...
# This is a complete pull of the beach seine data except that includes fishes only, no invertebrates. Can be considered--until the DJFMP EDI data are updated--a good source for the raw data.
seine_all_var <-
  dt3 %>% 
...
The goal here is to develop a beach seine data set using a reduced set of variables (not all are germane to this study), and pivot to wide format.

-   select variables

-   determine effort (number of seine hauls per site per year)
...
# save data file to local drive
# write_rds(seine_long, "~/Documents/projects/ds4eeb/Pete_local_data/seine.rds")
...
There are two factors to consider when selecting beach seine sites–which sites give us decent coverage in all salmon-relevant parts of the Delta, and which sites were sampled with sufficient consistency to ensure they provide a decent representation of the nearshore fish community.

Looking at mean effort, the average number of seine hauls performed at each site each year, I'm going to arbitrarily select only those sites whose mean equaled or exceeded 18, and hope that we'll get decent spatial coverage for the Delta.

```{r}

# form a df of sites matching that minimum effort (retains the station_code for each)
selected_sites <-
  mean_effort %>% 
  filter(mean >= 18) %>% 
  left_join(dt5, 
...
# save data files to local drive
# write_rds(selected_sites, "~/Documents/projects/ds4eeb/Pete_local_data/selected_sites.rds")
# write_rds(seine_long_min_effort, "~/Documents/projects/ds4eeb/Pete_local_data/seine_long_min_effort.rds")
```

### prep for analyses

Convert data from long format to wide, where each row is a sample unit, a single unique haul from a given location on a given date. Site location data (latitude, longitude) are in selected_sites.
...
# note that these data are a subset of the original data! retains data only from those sites where effort met the arbitrarily determined minimum

# stash data file to local folder
# write_rds(seine, "~/Documents/projects/ds4eeb/Pete_local_data/seine.rds")
...
```{r}
test <- slice_sample(seine[, c(3,4,8:12)], n = 25) %>% print()

test %>% group_by(year(date)) %>% summarise(chn = sum(chinook_salmon))

tule <- 
  seine %>% 
  group_by(year = year(date)) %>% 
  summarise(tule_perch = sum(tule_perch)) %>% 
  print(n = Inf)

library(ggplot2)
ggplot(tule, aes(x = year, y = tule_perch)) +
  geom_point()


seine %>% 
  mutate(year = year(date),
         tule.perch = integer('Tule Perch')) %>% 
  relocate(year, .before = date) %>% 
  select(year, tule.perch) %>% 
  group_by(year) %>% 
  summarise(tule.perch = sum(., na.rm = T))

seine[,8:114] %>% filter('No catch' != 0)
sum(seine[3,8:114])


  group_by(year) %>% 
  summarise(tule.perch = sum('Tule Perch', na.rm = T))



library(vegan)
# specnumber(seine[c(8:114)])
specnumber(unlist(seine[,c(8:114)])) # Number of species per site

diversity(seine[,8:114], index = "shannon") # Shannon index
diversity(seine, index = "simpson") # Simpson index
H <- diversity(seine[,8:114], "shannon")
S <- specnumber(seine[,8:114])
(J <- H / log(S))
```
...
```{r}
temp <- slice_sample(seine_all_var[, c(1,3,4,21,22,33)], n=25) %>% arrange(station_code)
temp %>% pivot_wider(names_from = iep_fish_code, values_from = count, values_fill = 0)
temp %>% pivot_wider(names_from = iep_fish_code, values_from = count, values_fill = 0, values_fn = sum) %>% print(n=25)




temp %>% filter(!grepl("unknown", common_name))

library(vegan)
specnumber(seine_w[1:6,c(3:4,8:11)])

temp <-
  seine_w[1:6,c(3:4,8:11)] %>% 
  tibble()

temp[c(2,4,5),c(5,6)] <- 0
temp[6,6] <- 0

as.numeric(unlist(temp$`Minnow unknown`))
